...
exports.check = function check (requestContext) {
  ...
  var curEtag = db.getRevision();

  // check if current etag and request etag missmatch
  // and if request url exists in cache
  if (reqEtag && reqEtag == curEtag && cache[requestContext.url] && options.cache) {

    if (cache[requestContext.url][reqEtag]) {
      // cache available
